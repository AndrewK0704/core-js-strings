...
function isPalindrome(str) {
  ...
  const arrayStr = str1.split('');
  const reverseArray = arrayStr.reverse();
  const joinArray = reverseArray.join('');
  return str1 === joinArray;
}
...
function findLongestWord(sentence) {
  ...
  let str = arrayStr[0];
  for (let i = 1; i < arrayStr.length; i += 1) {
    if (str.length < arrayStr[i].length) {
      str = arrayStr[i];
    }
  }
  return str;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arrayStr.length; i += 1) {
    const arrayStr2 = arrayStr[i].split('');
    const reverseArray = arrayStr2.reverse();
    const joinArray = reverseArray.join('');
    res.push(joinArray);
  }
  return res.join(' ');
